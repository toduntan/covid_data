...
echo %date% %time% [INFO] Executing db_grants.sql...
echo %date% %time% [INFO] Executing db_grants.sql... >> %logfile%
...
echo %date% %time% [INFO] db_grants.sql executed.
echo %date% %time% [INFO] db_grants.sql executed. >> %logfile%
...
snowsql -c primary -r SECURITYADMIN -q "alter user INGEST_SVC_ACCT set default_warehouse=INGESTION_WH default_namespace=INGESTION default_role=INGESTION_WRITE; grant role INGESTION_WRITE to user INGEST_SVC_ACCT;" -o output_file="%logfile%" -o friendly=false -o output_format=plain -o timing_in_output_file=true
